...
int findVictim(BufPool pool) {
    ...
    /*int count = 0;
    int currClock = pool->clock;*/
    while (slot == NONE) {
        /*if(currClock==pool->clock&&++count==MAX_USAGE+1){
            break;
        }*/
        // int clock = pool->clock;
        if (pool->bufs[pool->clock].pop == 0 && pool->bufs[pool->clock].pin == 0) {
            slot = pool->clock;
        } else if (pool->bufs[pool->clock].pop > 0) {
            pool->bufs[pool->clock].pop--;
        }
